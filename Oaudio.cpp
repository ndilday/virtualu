...
int Audio::init() {
    ...
    for(int i=0; i < MAX_LOOP_WAV_CH; ++i) {
...
int Audio::init_wav() {
    ...
    HRESULT rc=DirectSoundCreate8(NULL, &lp_direct_sound, NULL);
	...
	wav_init_flag=1;
	//DEBUG_LOG("Audio:: set wav_init_flag");
    }
...
int Audio::init_cd() {
	...
	cd_init_flag = 1;
	//DEBUG_LOG("Audio:: set cd_init_flag");
	return 1;
    }
...
// ## chwg0105
//------- Begin of function Audio::play_long_wav2 --------//
//!
void Audio::play_long_wav2(char *wavName, DsVolume dsVolume) {
    //stop the long_wav played before
    if(last_played_lwave_ch!=0)
	stop_long_wav(last_played_lwave_ch);
    last_played_lwave_ch=play_long_wav(wavName,dsVolume);
}

//------- End of function Audio::play_long_wav2 ----------//
...
void  Audio::yield() {
	...
	if(!writeTooFast) {
	    ...
	    // next bank to fill
	    lwav_bank[i] = (lwav_bank[i] + 1) % LWAV_BANKS;
	    if(DS_OK != hr) {
		// fail to lock
		err_now("Cannot lock DirectSoundBuffer");
		run_yield = 1;
	    ...
	    filePtr->file_read(lpvPtr1, dwBytes1);
	    // bytes read in
	    readStreamSize = filePtr->file_pos() - startPos;
	...
	if(!writeTooFast) {
	    ...
	    HRESULT hr = lpDsb->Lock(loopwav_bank[i]*LOOPWAV_STREAM_BUFSIZ, LOOPWAV_STREAM_BUFSIZ,
				     &lpvPtr1, &dwBytes1,  &lpvPtr2, &dwBytes2, 0);
	    // next bank to fill
	    loopwav_bank[i] = (loopwav_bank[i] + 1) % LOOPWAV_BANKS;
	    if(DS_OK != hr) {
		// fail to lock
		err_now("Cannot lock DirectSoundBuffer");
		run_yield = 1;
...
void Audio::stop_wav() {
    ...
    // ------------ stop all loop wave ------------//
    for(int i = 1; i <= MAX_LOOP_WAV_CH; ++i) {
	stop_loop_wav(i);
    }
...
// ------------ Begin of function Audio::int_to_DsVolume -------//
DsVolume Audio::int_to_DsVolume(int sys_config_vol) {
    if(sys_config_vol==0)
	return DsVolume(RelVolume(0,0));
    return DsVolume(RelVolume(long(sys_config_vol*0.2+80),0));
}

// ------------ End of function Audio::int_to_DsVolume -------//
