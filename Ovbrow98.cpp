...
char    VBrowse98::press_record=0;                // for detecting pulling records upwards and downloads
VBrowse98* VBrowse98::press_vbrowse_ptr;
...
void VBrowse98::deinit() {
    if(refresh_bitmap) {
	mem_del(refresh_bitmap);
	refresh_bitmap=NULL;
    }
}

//--------- Begin of function VBrowse98::init --------//
//!
...
//!                         will pass to it.
//! [int]	<BrowDetectFP> detectFunc	= Default as NULL, not implemented yet,
...
//!                         (default : MIN_SPACE)
//!
void VBrowse98::init(int inX1, int inY1, int inX2, int inY2,
		     int recWidth, int recHeight,
		     int totalRec, BrowDispFP dispFunc,
		     BrowDetectFP detectFunc,
	...
	err_now( "VBrowse98::init() error : disp_max_rec <= 0" );
...
//---------- End of function VBrowse98::init ---------//

//---------- Begin of function VBrowse98::init_var -------//
//!
//! Initialize variables for visual browser
//!
void VBrowse98::init_var(int totalRec, int recNo) {
    ...
    if(((recNo<top_rec_no)&&(recNo>top_rec_no+disp_max_rec-1))
       ||(init_flag==0)) {
	// Change top_rec_no
	top_rec_no = recNo - disp_max_rec/2;
	top_rec_no = max(1,top_rec_no);
	// if there is an empty area at the lower part of the browser
	if( top_rec_no+disp_max_rec-1 > total_rec_num ) {
    ...
    scroll_bar.init( 1, x2-SCROLL_BAR_WIDTH-7, y1+4, x2-7, y2-4, disp_max_rec,
		     (disp_frame && x_max_rec==1 ? 1 : disp_max_rec),// only use record as scroller unit when it's a vertical browser with frame
		     total_rec_num, 1 );
}

//----------- End of function VBrowse98::init_var ----------//

//--------- Begin of function VBrowse98::open ---------//
//!
//! Open the list box
//!
...
//!							  (default: no change)
//!
void VBrowse98::open(int recNo, int newTotalRec) {
...
//--------- End of function VBrowse98::open ---------//

//--------- Begin of function VBrowse98::close ---------//
//!
//! Close the list box, disable scroll bar and list content
//!
void VBrowse98::close() {
    state = 0;                                      // state = CLOSED

    //-------- paint the closed outlook of the list box -------//

    // the place a large block on the list content box
    Vga::active_buf->d3_panel_up( x1+1, y1+1, x2-SCROLL_BAR_WIDTH-3, y2-1  );
}

//--------- End of function VBrowse98::close ---------//

//--------- Begin of function VBrowse98::paint ---------//
//!
//! Display the movie list box
//!
void VBrowse98::paint() {
    vga.use_back();
    refresh_bitmap=vga_back.save_area( x1, y1, x2-SCROLL_BAR_WIDTH-3, y2,refresh_bitmap);
    scroll_bar.paint();
    sys.yield();                                    // chwg032799
}

//--------- End of function VBrowse98::paint ---------//

//------- Begin of function VBrowse98::refresh ---------//
//!
...
//!                       (default : -1, no change)
//!
void VBrowse98::refresh(int newRecNo, int newTotalRec) {
...
//---------- End of function VBrowse98::refresh ----------//

//------- Begin of function VBrowse98::update ---------//
//!
//! Yield all records which currently display in the vbrowse
//!
void VBrowse98::update() {
    int recNo, x, y;
...
//---------- End of function VBrowse98::update ----------//

//--------- Begin of function VBrowse98::detect ------//
//!
...
int VBrowse98::detect() {
    ...
    // only pull when the vbrowser is vertical, horizontal is technically possible, but not used practically
    if( detect_pull_flag && disp_frame && x_max_rec==1 ) {
	if( detect_pull() )
	    return rec_no;
    }
	...
	// if it is not in the empty browser area
	if( newRecNo>=top_rec_no && newRecNo<=top_rec_no-1+disp_max_rec ) {
	    if( mouse.click_count()==2 )                // double clicked
		double_click = 1;

	    if( newRecNo != rec_no ) {                  // if user point to a new record
		disp_one(rec_no, CLEAR_HIGH);
		disp_one(newRecNo , DISP_HIGH );          // 2 means display record content only
	...
	if( disp_frame && x_max_rec==1 ) {            // only use record as scroller unit when it's a vertical browser with frame, refer to scroll_bar.init() in VBrowse98::init_var()
	    if( rec_no < top_rec_no ) {
		top_rec_no = rec_no;
		disp_all();
	    }
...
//----------- End of function VBrowse98::detect -------//

//---------- Begin of function VBrowse98::detect_pull ---------//
//!
//! Detect pulling on record if it is a vertical browser with frame.
//!
int VBrowse98::detect_pull() {
...
//---------- End of function VBrowse98::detect_pull ---------//

//--------- Begin of function VBrowse98::detect_right ------//
//!
...
//!                =0 if no action
//!
int VBrowse98::detect_right() {
    if( state == 0 || none_record )                 // closed
	return 0;

    // 1-right mouse button
    if( mouse.single_click( ix1, iy1, ix2, iy2, 1 ) ) {
	int recNo;
...
//----------- End of function VBrowse98::detect_right -------//

//-------- Begin of function VBrowse98::disp_all ----------//
//!
//! Display all records on screen, highlight cur_rec_no
//!
void VBrowse98::disp_all() {
    int recNo;
...
//--------- End of function VBrowse98::disp_all ----------//

//-------- Begin of function VBrowse98::disp_one ----------//
//!
...
//!
void VBrowse98::disp_one(int recNo, int dispType) {
    if( none_record )
	return;
	...
	vga_back.rest_area_with_clipping(refresh_bitmap,
					 x-2-x1,y-2-y1,x+rec_width-3-x1, y+rec_height+1-y1,
					 0,1);
	disp_rec( recNo, x, y, INFO_REPAINT );
	vga.blt_buf(x-2,y-2,x+rec_width-3,y+rec_height-1);
    }
	...
	vga_back.rest_area_with_clipping(refresh_bitmap,
					 ...
					 0,1);
	disp_rec( recNo, x, y, INFO_REPAINT );
	user_interface.darken(x-2,y+rec_height-2,x+rec_width-7,y+rec_height-1);
	user_interface.darken(x+rec_width-9,y-2,x+rec_width-7,y+rec_height-3);
	user_interface.darken(x-2,y-2,x+rec_width-7,y+rec_height-1);
	vga.blt_buf(x-2,y-2,x+rec_width-7,y+rec_height-1);
...
//--------- End of function VBrowse98::disp_one ----------//

//-------- Begin of function VBrowse98::disp_rec --------//
//!
void VBrowse98::disp_rec(int recNo, int x, int y, int refreshFlag) {
    vga.use_back();
    vga_back.rest_area_with_clipping(refresh_bitmap,
				     0,y-y1,(rec_width-SCROLL_BAR_WIDTH+12),rec_height+y-y1,
				     0,1);
    (*disp_rec_func)( recNo, x, y, refreshFlag );   // call user defined function
}

//----------- End of function VBrowse98::disp_rec -----------//
