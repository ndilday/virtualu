...
//#define HELP_INACTIVE_TIME  ((float) 0.5)		// when the mouse is inactive for one second, display help

//##### begin trevor 10-12 #####//
#define HELP_INACTIVE_TIME  ((float) 1.5)         // when the mouse is inactive for one second, display help
//##### end trevor 10-12 #####//

enum {                                            // 400
    HELP_SCR_BUF_WIDTH  = 400,
...
void Help::load(char* helpFileName) {
	...
	// allocate a buffer larger than we need for the largest size possible
	help_text_buf      = mem_resize( help_text_buf, dataSize );
	...
	//------ if it's a help area position ------//
	    ...
	    // 0 when display for both interfaces
	    iptr->monster_human_interface = (short) fileTxt.get_num();
...
//---------- End of function Help::set_help2 ---------//
...
void Help::disp_short_help(VgaBuf *vgaBuf) {
	...
	*/
	...
	vga.use_back();
	if(short_back_buf.save_scr_buf)
	    short_back_buf.rest_scr();                  // ## chwg031199

	// save buffer

	//		(vgaBuf->is_front ? short_front_buf : short_back_buf).save_scr( x1, y1, x2, y2, vgaBuf );// ## chwg031199

	// ## chwg031199
	short_back_buf.save_scr( x1, y1, x2, y2, &vga_front );
