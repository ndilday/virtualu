//Filename    : OGRPH_PM.H
//Description : Header of object Performance Indicator of Month
...
//--------- Define class GraphPerformanceIndicatorMonth --------//
//
//!
class GraphPerformanceIndicatorMonth {
public:
...
public:
    ...
    void init(int,int,int,int,int,int*,void*,char,char=1,char=1,int=1,int* =NULL,int=Vga::active_buf->color_dark);
	...
	init(x1, y1, x2, y2, seriesNum, dataNum,
	     (void*) dataArray, DATA_FLOAT,
	     transparentFlag, valueFlag,
	     numFormat, seriesColor, axisColor);
    }
