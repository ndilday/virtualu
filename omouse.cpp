...
//--- define the size of a buffer for real-time vga screen updating ---//
                                                  ...
                                                  // 100 x 100
#define VGA_UPDATE_BUF_SIZE (100*100*sizeof(short))
...
// coordination of the last double-buffer update area
static int update_x1, update_y1, update_x2, update_y2;
...
//------------ Start of Mouse::init ------------//
//
void Mouse::init(HINSTANCE hinst, HWND hwnd, IDirectInput8* createdDirectInput) {
    ...
    */
	...
	hr = di_mouse_device->SetCooperativeLevel(hwnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE);
    else
...
int Mouse::press_area(int x1, int y1, int x2, int y2, int buttonId) {
	...
	// Left button
	if( left_press && (buttonId==0 || buttonId==2) )
	    return 1;

	// Right button
	if( right_press && (buttonId==1 || buttonId==2) )
	    return 2;
    }
...
//          0 - if not
//
int Mouse::single_click(int buttonId) {
    if( !has_mouse_event )
	return 0;
...
void Mouse::poll_event() {
	...
	if( sys.use_true_front == 2 ) {               // window mode
		...
		&& ScreenToClient( sys.main_hwnd, &cursorPos )
		&& cursorPos.x >= bound_x1 && cursorPos.x <= bound_x2
		&& cursorPos.y >= bound_y1 && cursorPos.y <= bound_y2 ) {
		cur_x = cursorPos.x;
		cur_y = cursorPos.y;
	    }
	}
...
int Mouse::wait_press(int timeOutSecond) {
    // avoid repeat clicking
    while( mouse.left_press || mouse.any_click() || mouse.key_code ) {
	sys.yield();
	mouse.get_event();
    }
...
int Mouse::is_key(unsigned scanCode, unsigned short skeyState, unsigned short charValue, unsigned flags) {
	...
	// backspace
    case DIK_BACK: priChar = capitalChar = shiftChar = KEY_BACK_SPACE; break;
	...
	// * on numeric keypad
    case DIK_MULTIPLY: priChar = capitalChar = shiftChar = '*'; onNumPad = 1; break;
    case DIK_SPACE: priChar = capitalChar = shiftChar = ' '; break;
	// + on numeric keypad
    case DIK_ADD: priChar = capitalChar = shiftChar = '+'; onNumPad = 1; break;
	// / on numeric keypad
    case DIK_DIVIDE: priChar = capitalChar = shiftChar = '/'; onNumPad = 1; break;
	// - on numeric keypad
    case DIK_SUBTRACT: priChar = capitalChar = shiftChar = '-'; onNumPad = 1; break;
