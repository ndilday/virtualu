...
void VBrowse::paint() {
    ...
    Vga::active_buf->d3_panel_down( x1, y1, x2-SCROLL_BAR_WIDTH-3, y2 );
...
int VBrowse::detect() {
	...
	if( recNo>0 && recNo<=top_rec_no-1+disp_max_rec ) {
...
void VBrowse::disp_all() {
    ...
    // clear background
    Vga::active_buf->bar( ix1, iy1, ix2, iy2, VBROWSE_COLOR_BACK );
...
void VBrowse::disp_one(int recNo, int dispType) {
    ...
    //		vga_front.d3_panel_up( x-2, y-2, x+rec_width-2, y+rec_height-2, 2 );
	...
	// ## chwg1111
	user_interface.adjust_brightness( x-2, y-2, x+rec_width-2, y+rec_height-2,-2);
    }
}
