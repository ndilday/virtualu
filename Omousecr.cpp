...
void MouseCursor::set_icon(int cursorId) {
    if(box.progress_val>0)
	if(info.prerun_year==1) {
	    cursorId=CURSOR_WAITING;
	}
...
void MouseCursor::process(int curX, int curY) {
	...
	vga_front.put_bitmapW( max(cur_x1,0), max(cur_y1,0), save_scr->bitmap_ptr() );

    //Kevin ::0909 restore the stored area to back buffer
    ////	vga_back.put_bitmapW( curX, curY,save_back_scr->bitmap_ptr() );
	...
	if( cur_x1 < 0 || cur_x2 >= VGA_WIDTH || cur_y1 < 0 || cur_y2 >= VGA_HEIGHT ) {
	    vga_front.read_bitmapW( max(cur_x1,0), max(cur_y1,0),
				    min(cur_x2,VGA_WIDTH-1), min(cur_y2,VGA_HEIGHT-1), save_scr->bitmap_ptr() );
	    ...
	    // must use PutIcon instead of PutArea for background masking
	    vga_front.put_bitmap_trans( cur_x1, cur_y1, icon_ptr );
	}
...
void MouseCursor::process_frame(int curX, int curY) {
    ...
    if( frame_shown ) {
	vga_front.put_bitmapW( frame_x1, frame_y1, frame_top_save_scr    );
	vga_front.put_bitmapW( frame_x1, frame_y2, frame_bottom_save_scr );
	vga_front.put_bitmapW( frame_x1, frame_y1, frame_left_save_scr   );
	vga_front.put_bitmapW( frame_x2, frame_y1, frame_right_save_scr  );
    }
...
void MouseCursor::disp_back_buf(int bltX1, int bltY1, int bltX2, int bltY2) {
	...
	//--- save the back buffer area which will be overwritten ---//
	...
	IMGbltW( save_scr->buf_ptr(), sizeof(short) * (min(cur_x2,VGA_WIDTH-1) -max(cur_x1,0)+1), x1-max(cur_x1,0), y1-max(cur_y1,0), save_back_scr->bitmap_ptr() );
