...
void ScrollBar::paint(int topRecNo) {
    ...
    if( type == VERTICAL ) {
	// up button
	Vga::active_buf->d3_panel_up( x1+2, y1+2 , x2-2, y1+13);
	// down button
	Vga::active_buf->d3_panel_up( x1+2, y2-13, x2-2, y2-2 );
    }
    else {
	// left button
	Vga::active_buf->d3_panel_up( x1+2 , y1+2, x1+13, y2-2);
	// right button
	Vga::active_buf->d3_panel_up( x2-13, y1+2, x2-2, y2-2 );
    }

    //--------- display cursor bitmap ----------//
    if(vga.use_back_buf) {
	if( type == VERTICAL ) {
	    //			image_sys.put_back( x1+4, y1+4 , "SCROLL_U" );
	    //			image_sys.put_back( x1+4, y2-11, "SCROLL_D" );
	    image_interface.put_back( x1+2, y1+2 , "SPIN_U_O" );
	    image_interface.put_back( x1+2, y2-13, "SPIN_D_O" );
	}
	else {
	    image_sys.put_back( x1+4 , y1+4, "SCROLL_L" );
	    image_sys.put_back( x2-11, y1+4, "SCROLL_R" );
	}
    }
    else {
	if( type == VERTICAL ) {
	    //			image_sys.put_front( x1+4, y1+4 , "SCROLL_U" );
	    //			image_sys.put_front( x1+4, y2-11, "SCROLL_D" );
	    image_interface.put_front( x1+2, y1+2 , "SPIN_U_O" );
	    image_interface.put_front( x1+2, y2-13, "SPIN_D_O" );
	}
	else {
	    image_sys.put_front( x1+4 , y1+4, "SCROLL_L" );
	    image_sys.put_front( x2-11, y1+4, "SCROLL_R" );
	}
    }
...
void ScrollBar::refresh(int topRecNo, int forceRefresh, int pageSkipRec, int dispMaxRec, int totalRec) {
	...
	indicator_y = newY;
	if( type == VERTICAL ) {
	    vga_front.d3_panel_down_clear( x1, y1+12, x2, y2-13 );
	    vga_front.d3_panel_up( x1+2, indicator_y, x2-2, indicator_y+indicator_height-1 )
		;
	    vga_back.d3_panel_down_clear( x1, y1+12, x2, y2-13 );
	    vga_back.d3_panel_up( x1+2, indicator_y, x2-2, indicator_y+indicator_height-1 );
	    //			image_interface.put_front( x1+2, indicator_y , "SPIN_DIS" );
	    //			image_interface.put_back( x1+2, indicator_y , "SPIN_DIS" );
	}
	else {
	    vga_front.d3_panel_down_clear( x1+12, y1, x2-13, y2 );
	    vga_front.d3_panel_up( indicator_y, y1+2, indicator_y+indicator_height-1, y2-2 );
	    vga_back.d3_panel_down_clear( x1+12, y1, x2-13, y2 );
	    vga_back.d3_panel_up( indicator_y, y1+2, indicator_y+indicator_height-1, y2-2 );
	    //			image_interface.put_front( indicator_y,y1+2, "SPIN_DIS" );
	    //			image_interface.put_back( indicator_y,y1+2, "SPIN_DIS" );
	}
...
int ScrollBar::detect() {
	...
	//||mouse.key_code == KEY_UP || mouse.key_code == KEY_LEFT
	) {
	if( --topRecNo < 1 )
	    topRecNo = 1;
	...
	//||mouse.key_code == KEY_DOWN || mouse.key_code == KEY_RIGHT
	) {
	...
	// page up
	if( pos < indicator_y || mouse.key_code == KEY_PGUP ) {
	    if( ( topRecNo -= skip_rec_num ) < 1 )
		topRecNo = 1;
	}

	// page down
	else if( pos >= indicator_y+indicator_height || mouse.key_code == KEY_PGDN ) {
	...
	while( mouse.left_press ) {
		...
		if( mouse.left_press && lastMouseY != mouse.cur_y ) {
		    ...
		    // scroll panel
		    vga_front.d3_panel_down_clear( x1, y1+12, x2, y2-13 );
		    vga_front.d3_panel_down( x1+2, indicator_y, x2-2, indicator_y+indicator_height-1 );
		    // scroll panel
		    vga_back.d3_panel_down_clear( x1, y1+12, x2, y2-13 );
		    vga_back.d3_panel_down( x1+2, indicator_y, x2-2, indicator_y+indicator_height-1 );
		...
		if( mouse.left_press && lastMouseY != mouse.cur_x ) {
		    ...
		    vga_front.d3_panel_down_clear( x1+12, y1, x2-13, y2 );
		    vga_front.d3_panel_down ( indicator_y, y1+2, indicator_y+indicator_height-1, y2-2 );
		    vga_back.d3_panel_down_clear( x1+12, y1, x2-13, y2 );
		    vga_back.d3_panel_down ( indicator_y, y1+2, indicator_y+indicator_height-1, y2-2 );
