...
namespace NEWMAT {
	...
	Select = Exception::Select;
	AddMessage("detected by Newmat: can't convert matrix to vector\n\n");
	Tracer::AddTrace();
    }
	...
	Select = Exception::Select;
	AddMessage("detected by Newmat: incompatible dimensions\n\n");
	MatrixDetails(A); MatrixDetails(A); MatrixDetails(B);
	Tracer::AddTrace();
    }
	...
	: Logic_error() {
	Select = Exception::Select;
	AddMessage("detected by Newmat: can't build matrix type ");
