...
#define MONITOR(what,store,storage) {}
    ...
    // Assume can't have both !DirectPart && StoreHere for MatrixRowCol routines
    ...
    void Matrix::NextRow(MatrixRowCol& mrc) {       // 1808
	REPORT mrc.IncrMat();
    }
	...
	REPORT
	    ...
	    if (+(mrc.cw*StoreHere))                      // shouldn't happen
	...
	REPORT                                        //571
		...
		// don't allow StoreOnExit and !DirectPart
		if (+(mrc.cw*StoreOnExit))
    ...
    // Don't need RestoreRow because we don't allow !DirectPart && StoreOnExit
