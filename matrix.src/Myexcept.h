...
namespace RBD_COMMON {
    ...
    //!Class for outputting trace or adding it to a thrown exception. Part of
    //!the exception handling package defined in Myexcept.h
    ...
    //!Superclass for exceptions, which are thrown when recoverable errors are
    //!found at runtime.
...
#include <setjmp.h>
    ...
    //!Class used to implement exceptions. Part of the Myexcept library. A
    //!Pointer to a linked list of jmp_buf s.
    ...
    //!Class used to implement exceptions. Part of the Myexcept library. An
    //!item in a linked list of jmp_buf s - takes care of trace and cleanup.
...
//****************** cleanup heap following Throw ***********************//
  ...
  //!Helper class for simulating exceptions. Used to cleanup heap following
  //!Throw. Part of Myexcept library.
class Janitor {
protected:
...
// declared globally
  ...
  //!Initialization class needed for classes derived from Janitor which have
  //!objects declared globally.
...
//!Class used by FreeCheck to register and report on memory allocation
//!calls.
class FreeCheckLink {
protected:
FreeCheckLink* next;
...
//!FreeCheckLink for registering classes. Part of suite of routines which
//!performs memory leak detection (traces balanced new/delete calls) when
//!DO_FREE_CHECK is defined.
...
//!FreeCheckLink for registering real arrays. Part of suite of routines
//!which performs memory leak detection (traces balanced new/delete calls)
//!when DO_FREE_CHECK is defined.
...
//!FreeCheckLink for registering int arrays. Part of suite of routines
//!which performs memory leak detection (traces balanced new/delete calls)
//!when DO_FREE_CHECK is defined.
...
//!Class which contains a set of routines for tracing whether new and
//!delete calls are balanced.
class FreeCheck {
...
//!Base class for exceptions defined as runtime errors. Defined in
//!Myexcept.h.
...
//!Logic error defined in Myexcept library for matrix operations.
...
//!Runtime error. Part of the Myexcpt library.
...
//!Runtime error defined in Myexcept library.
