...
#define REPORT {}
    ...
    ElmDivideMatrix& ElmDivide(const BaseMatrix& bm1,const BaseMatrix& bm2) {
	REPORT
	    ElmDivideMatrix* x = new ElmDivideMatrix(&bm1, &bm2);
	MatrixErrorNoSpace(x);
	return *x;
    }
    ...
    ElmDivideMatrix ElmDivide(const BaseMatrix& bm1,const BaseMatrix& bm2)
    { REPORT return ElmDivideMatrix(&bm1, &bm2); }
	...
	REPORT int i=storage; Real* s=store; while (i--) {
	    *s++ = f;
	}
    }
