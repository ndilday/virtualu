...
namespace NEWMAT {
	...
	enum State {
	    Start, Restart, Continue, Interpolate, Extrapolate,
	    Fail, Convergence
	};
	...
	if (!Pred.IsValid()) {                        // check afterwards as well
	    oorg=true; return;
	}
	Y = *DataPointer - Y; Real ssq = Y.SumSquare();
	errorvar =  ssq / (n_obs - n_param);
	std::cout << "\n" << std::setw(15) << std::setprecision(10) << " " << errorvar;
	...
	v = LL.LogLikelihood();
	if (!LL.IsValid()) {                          // check validity again
	    oorg=true; return;
	}
	std::cout << "\n" << std::setw(20) << std::setprecision(10) << v;
	oorg = false;
	...
	Adj = LT.t().i() * Adj1;
	test = SumSquare(Adj1);
	std::cout << "   " << std::setw(20) << std::setprecision(10) << test;
