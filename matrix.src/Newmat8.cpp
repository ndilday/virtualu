...
#define REPORT {}
    ...
    Real GeneralMatrix::MaximumValue() const {
	REPORT
	    Real maxval = 0.0; int i = storage; Real* s = store;
	    while (i--) { Real a = *s++; if (maxval < a) maxval = a; }
	    ((GeneralMatrix&)*this).tDelete(); return maxval;
    }

    Real GeneralMatrix::MinimumValue() const {
	REPORT
	    Real minval = 0.0; int i = storage; Real* s = store;
	    while (i--) { Real a = *s++; if (minval > a) minval = a; }
    ...
    Real BaseMatrix::MaximumValue() const {
	REPORT GeneralMatrix* gm = ((BaseMatrix&)*this).Evaluate();
	Real s = gm->MaximumValue(); return s;
    }
    Real BaseMatrix::MinimumValue() const {
	REPORT GeneralMatrix* gm = ((BaseMatrix&)*this).Evaluate();
	Real s = gm->MinimumValue(); return s;
    }
