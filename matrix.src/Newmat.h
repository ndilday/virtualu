...
#define ReturnMatrix ReturnMatrixX&
    ...
    //!Return from LogDeterminant function - value of the log plus the sign (+,
    //!- or 0). Part of the newmat library.
    class LogAndSign {
    ...
    //!Class to show whether to check for loss of data. Defined in newmat
    //!library.
    ...
    //!To turn off conversion checking (whether to check for loss of data)
    //!within the current block. Defined in newmat library.
    ...
    //!Is used for finding the type of a matrix resulting from the binary
    //!operations +, -, * and identifying what conversions are permissible.
    //!Defined in newmat library.
    class MatrixType {
    public:
	enum Attribute {
	    Valid     = 1,
	...
	static int nTypes() {                           // number of different types
	    return 9;
	}
	// exclude Ct, US, BC
    public:
    ...
    //!Class for representing the Band Width of a matrix. Defined in newmat
    //!library.
    class MatrixBandWidth {
    public:
	int lower;
    ...
    //!This class is introduced to avoid constructors such as
    //!ColumnVector(int). Used for conversions.
	...
	virtual Real MaximumValue() const;          // Fred 0512
	virtual Real MinimumValue() const;          // fred 0512
    ...
    //!General purpose matrix class, as defined in newmat library.
	...
	Real MaximumValue() const;                  // Fred 0512
	Real MinimumValue() const;                  // Fred 0512
	...
	GeneralMatrix* Transpose(TransposedMatrix*, MatrixType);
	void ReSize(int);                           // change dimensions
	void set_diagonal(const ColumnVector& );    // fred 0512
	Real* nric() const {
	    // for use by NRIC
    ...
    //!Matrix subclass for 1 x N vectors.
    ...
    //!Generic purpose matrix class, as defined in newmat library.
    ...
    //!Class for representing a multiplied matrix. Defined in newmat library.
    ...
    // class added by Fred 0512
    //!Matrix subclass
    class ElmDivideMatrix : public SPMatrix {
    protected:
	ElmDivideMatrix(const BaseMatrix* bm1x, const BaseMatrix* bm2x)
	    : SPMatrix(bm1x,bm2x) {}
...
#ifndef TEMPS_DESTROYED_QUICKLY
	friend ElmDivideMatrix ElmDivide(const BaseMatrix&, const BaseMatrix&);
#else
	friend ElmDivideMatrix& ElmDivide(const BaseMatrix&, const BaseMatrix&);
#endif

	NEW_DELETE(ElmDivideMatrix)
	    };

    //!Matrix subclass for concatenated matrices.
    ...
    //!Class for representing a stacked matrix. Used in newmat library.
    ...
    //!Class for representing a negated matrix. Defined in newmat library.
    ...
    //!Class for representing a Scaled matrix. Used in newmat library.
    ...
    //!Class for representing a negative shifted matrix. Defined in newmat
    //!library.
    ...
    //!Class for representing a reversed matrix. Used in newmat library.
    ...
    //!Class for representing an inverted matrix. Defined in newmat library.
    ...
    //!Matrix subclass for diagonal matrices?
    ...
    //!Class for representing a mated matrix. Defined in newmat library.
    ...
    //!Class for returning matrix after a calculation. Used in newmat library.
    ...
    //!Class for getting the sub matrix. Defined in newmat library.
	...
	void operator-=(Real r) {                   // subtract constant
	    operator+=(-r);
	}
	void operator*=(Real);                      // multiply by constant
	void operator/=(Real r) {                   // divide by constant
	    operator*=(1.0/r);
	}
	void Inject(const GeneralMatrix&);          // copy stored els only
	MatrixBandWidth BandWidth() const;
    ...
    //!Class used for reading a list of values into a matrix. Defined in newmat
    //!library.
    ...
    //!Runtime error (Not positive definite) defined in newmat library for
    //!matrix operations.
    ...
    //!Runtime error defined in newmat library.
    ...
    //!Logic error defined in newmat library.
    ...
    //!Logic error defined in newmat library for matrix operations.
    ...
    //!Logic error defined in newmat library for matrix operations - can't
    //!convert to vector.
    ...
    //!An exception wrapper for C++. The program cannot build a matrix.
