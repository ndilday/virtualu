...
void VBrowseIF::paint() {
    ...
    vga.d3_panel_down( x1, y1, x2-SCROLL_BAR_WIDTH-3, y2, vga_front_only );
...
void VBrowseIF::refresh(int newRecNo, int newTotalRec) {
	...
	// the list content box
	vga.blt_buf( x1, y1, x2-SCROLL_BAR_WIDTH-3, y2 );
...
void VBrowseIF::disp_one(int recNo, int dispType) {
	...
	// top
	vga.blt_buf( x-2, y-2       , x+rec_width+1, y-2         );
	// bottom
	vga.blt_buf( x-2, y+rec_height+1, x+rec_width+1, y+rec_height+1 );
	// left
	vga.blt_buf( x-2, y-2       , x-2        , y+rec_height+1 );
	// right
	vga.blt_buf( x+rec_width+1, y-2 , x+rec_width+1, y+rec_height+1 );
    }
