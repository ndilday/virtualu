...
#include <stdio.h>
#include <ALL.H>                                  // class Memory is declared here
...
//!Structure containing header and debuggin information for allocated
//!blocks.
struct MemInfo {
...
// global debug function to display stat in mem.info_array
size_t mem_usage() {
    size_t memUsed = 0;
    for( int i=0; i<mem.ptr_used;++i ) {
	memUsed += mem.info_array[i].size;
    }
    return memUsed;
}
...
char* Mem::add_clear(unsigned memSize, char* fileName, int fileLine) {
    err_when( memSize > 0x800000 );                 //**BUGHERE, for temporary debugging only
...
char* Mem::resize(void *orgPtr, unsigned memSize, char* fileName, int fileLine) {
    //	err_when( memSize > 0x800000 );		//**BUGHERE, for temporary debugging only		//## fred 1204

    if( orgPtr == NULL && memSize == 0 )
	return NULL;
	...
	if( info_array[i].ptr == orgPtr ) {
		...
		newPtr = new char[memSize+CHK_VAL_SIZE*2];

		if( newPtr == NULL )
		    err.mem();

		memcpy( newPtr, (char *)orgPtr-CHK_VAL_SIZE, CHK_VAL_SIZE+min(memSize,info_array[i].size) );
		delete[] ((char *)orgPtr -CHK_VAL_SIZE );
...
#ifdef DEBUG
    if( !info_array ) {
	err.msg( "Mem::del - Free memory after ~Mem, File name:%s, line no.:%d\n", fileName, fileLine );
	return;
    }
#endif
...
Mem::~Mem() {
    ...
    delete[] info_array;
    info_array = NULL;                              // to mark sub-sequence mem_del issue a message
}
