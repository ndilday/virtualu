...
//!                    5 - $1,000K, thousand seperator, no decimal place and dollar sign
//!                    7 - $1,000,000, for surplus display! Font diamond only!
//!                    (default : 1)
//!                    8 - -10%	'-' sign for negative value
//!                    9 - display the 1/1000 of the real value.(showing 0 dec place.)
//!                    10 - display the 1/100 of the real value. (showing 2 dec place.)
//!                    11 - display the 1/1000 of the real value.(showing 2 dec place.)
//!                    12 - display the 1/1000 of the real value.(showing 0 dec place.) with 'K'
//!                    13 - (1000)
//!                    14 - 100hrs
//!                    17 - 100hrs/week  //## chea 180999 not use yet...
//!                    15 - display the 1/10 of the real value. (showing 1 dec place.)
//!                    16 - display in the format "Yr.2", "Yr.3"
//!							 28 - (1,000,000)						// Kenneth
//!							 29 - (56%)								//marco
//!							 30 - display the 1/1000 of the real value. (showing 1 dec place.) with 'K'
//!
...
char* Misc::format( int inNum, int formatType ) {
    ...
    // -------- shift left 3 digits and have 1 decimal place and have a 'K' ----------- //
    if ( formatType == 30 ) {
	return format( ((double)(inNum)/1000), 30 );
    }

    //--------- shift left 3 digits and no dollar sign----------//
    if( formatType == 9 || formatType == 12) {
	inNum=inNum/1000;
    }
    //----------------------//

    //--------- shift left 2 digits and add dollar sign----------//
    if( formatType == 15 ) {
	return format((double)(((sign==-1)?-1:1)*inNum/100.0),4);
    }
    //----------------------//
    ...
    //--------- shift left 3 digits and add dollar sign----------//
    if( formatType == 11 ) {
	return format((double)(((sign==-1)?-1:1)*inNum/1000.0),4);
    }
    //----------------------//
	...
	if(inNum>1000)
	    return format((double)(((sign==-1)?-1:1)*inNum),5);
	*outPtr++ = '$';
    }
    //----------------------//
	...
	*outPtr++ = '$';
    ...
    if( formatType == 8 || formatType == 13 || formatType == 28 || formatType == 29) {
	if( sign < 0 )
	    *outPtr++ = '-';
    }
    else
    ...
    if( formatType == 13 || formatType == 28 || formatType == 29)
	*outPtr++ = '(';
	...
	*outPtr++ = '$';
    ...
    //## chea 5.1.2
    if( formatType == 16) {                         //## chea 061199 finance 2.5.1
	//		if(inNum<10 && i==1)
	//		{
	//			*outPtr++ ='0';
	//		}
	...
	*outPtr++ = '%';
    ...
    //--------- K sign ------------//
    if( formatType == 5 || formatType == 12 || formatType == 30 )
	*outPtr++ = 'K';

    //--------- negetive bracket ----------//

    if( formatType != 8 && formatType != 13)
	if( sign < 0 )
	    *outPtr++ = ')';
	...
	*outPtr++ = ')';
    ...
    if( formatType == 14 || formatType == 17) {
	*outPtr++ = 'h';
	*outPtr++ = 'r';
...
//! [int] formatType = 1 - 1,000,000  add thousand seperator
//!                    2 - $1,000,000 add thousand seperator and dollar sign
//!                    3 - 56% add percentage sign % at the end of the number (0 dec)
//!							 5 - $1,000K thousand seperator, no decimal place and dollar sign
//!                    7 - $1,000,000, for surplus display! Font diamond only!
//!							 8 - -56.2% show '-' for negative number, (1 DEC)
//!							 9 - show integer part only
//!							 13 - (1000.0)					//marco
//!                    14 - 100hrs
//!                    15 - 100hrs/week
//!                    16 - 100hrs/week (with 1 dec)
//!                    23 - 23.32 (2 dec places. no %)
//!                    24 - 23.3% (1 dec places. & %)
//!                    25 - 23.32% (2 dec places  with %)
//!                    26 - 23.3 (1 dec place)
//!							 27 - ($1,000,000)
//!							 28 - (1,000,000)
//!							 29 - (23%)
//!							 30 - 23.3K (1 dec place with K)
//! return <char*> the pointer to the converted string, the string
//!                is stored in static variable which will be overwritten
//!                in next call.
//!
char* Misc::format(double inNum, int formatType) {
	...
	//		if((inNum>=1000000)||(inNum<=-1000000))
	//		{	inNum/=1000000.0;
	//			endingChar='M';
	//		}
	//		else
	if((inNum>=1000)||(inNum<=-1000)) {
	    inNum/=1000.0;
	    endingChar=' ';
	}
	*outPtr++ = '$';
	...
	*outPtr++ = '$';
    ...
    // ##### Begin MArco ##### // (for 1 dec place)
    if ( formatType == 8 || formatType == 13 || formatType == 16 || formatType == 24 || formatType == 26 || formatType == 30)
	floatStr = fcvt( inNum, 1, &intDigit, &sign );
    else
    ...
    // #### Begin Marco ##### //
    if( formatType == 13 || formatType == 27 || formatType == 28|| formatType == 29)
	*outPtr++ = '(';
    // #### End Marco ##### //
    //--------- dollar sign ($) ------------//
    if( formatType == 2 || formatType == 27)
	*outPtr++ = '$';
	...
	*outPtr++ = '0';
    ...
    //------- dec. place number -----------//
    /*
      if(((formatType == 3 )&&(fabs(inNum-(int)inNum)>1e-2))
      ||(formatType != 3 )
      //		&&(endingChar !='K')&&(endingChar !='M'))
      )
      //		if(( inNum > -10) && (inNum < 10 ))   // if the number is less than 1000, add dec. places
    */

    //## chea 110899
    /*Begin Marco*/
    if(formatType == 24 || formatType == 26 || formatType == 13/*End Marco*/ || formatType == 30) {
	...
	*outPtr++ = 'K';
	...
	||(formatType == 28)
	//			&&( inNum > -10) && (inNum < 10 ))
	//			||((formatType != 2 )&&(formatType != 3 )&&(formatType != 9 )&&(formatType != 7 ))
	) {
	// if the number is greater than 1000, truncate any dec. places
	*outPtr++ = '.';
	...
	//## chea 180999
	if(formatType!=8 && formatType!=14 && formatType!=15 && formatType != 16)
	    if( *floatStr )                             // 2nd dec. place
		*outPtr++ = *floatStr++;
	    else
		*outPtr++ = '0';

	//			if((*(outPtr-1)=='0') && (*(outPtr-2)=='0'))	// e.g. 0.00 -> 0
	//				outPtr-=3;
	else if((formatType == 7 )&&(inNum<0))
	    *(outPtr-1)+=111;                           // turn the number of f-diamond to red

    }

    //--------- percent sign (%) ------------//

    if( formatType == 3 || formatType == 8 || formatType == 24 || formatType == 25 || formatType == 29) {
	*outPtr++ = '%';
    }
    //--------- negetive bracket ------------//
	...
	*outPtr++ = endingChar;
    ...
    if( formatType == 14 || formatType == 16 || formatType == 15) {
	*outPtr++ = 'h';
	*outPtr++ = 'r';
    ...
    if( formatType == 16 || formatType == 15) {
	*outPtr++ = '/';
	*outPtr++ = 'w';
	*outPtr++ = 'e';
	*outPtr++ = 'e';
...
//------- Begin of function Misc::atof ---------//
//!
//! Same as atof() in stdlib.h instead it allows you to specify the
//! length of the string
//!
//! <char*> str    = the string to be converted to integer
//! <int>   strLen = length of the string
//!
double Misc::atof( char *str, int strLen ) {
    if ( strLen >= sizeof( str_buf ) )
	strLen = sizeof( str_buf ) - 1 ;
...
#define RANDOM_MAX      0x7FFFU
    ...
    else {                                          //if ( maxNum <= 0x7FFF )	//BUGHERE
	return 16 * random(maxNum/16);
    }
}
...
// ##### Begin Marco ##### //
//------- Begin of function Misc::round_1dec ---------//
//!
//! Round the number to its nearest 1 decimal places.
//!
//! <float> inNum - the input number.
//!
//! Return : <float> the output number
//!
float Misc::round_one_dec(float inNum) {
    return (float)((int)(inNum * 10)) / 10;
}
