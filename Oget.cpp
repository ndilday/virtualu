...
void Get::field(int desX, int y1, char* fldDes, int fldX1, char* dataPtr,
  ...
  //-------------------------------------//
    ...
    // field description
    vga.d3_panel_up( desX, fld_y1, fld_x1, fld_y2, 1 );

    font_ptr->put( desX+3, fld_y1+3, fldDes );

                                                  // field content
    vga.d3_panel_down( fld_x1+4, fld_y1, fld_x2, fld_y2 );
...
void Get::field(int x1, int y1, char* dataPtr, int dataWidth, int x2) {
    ...
    // field content
    vga.d3_panel_down( fld_x1, fld_y1, fld_x2, fld_y2 );
...
int Get::load_data( GetFld *getPtr, char *chr_buf ) {
    ...
    */
    ...
    if( mask_flag )
	// masked the field for displaying hidden data (like password)
	memset( chr_buf, '*', strlen((char*)getPtr->data) );
    else
	// don't copy the NULL character
	memcpy( chr_buf, (char*)getPtr->data, strlen((char*)getPtr->data) );
...
int Get::read(int firstRead) {
	...
	if ( keyCode >= 0x20 && keyCode <= 0xFF ) {
		...
		// If the firstFlag key press is not cursor key, clear the field content, it can be any one of the types c,i,l,d,N
		if ( m.str_chr("cildNC", getPtr->type) ) {
		    if ( firstKey )
...
int Get::get_next_key( char* dataBuf, char* pictBuf, GetFld* getPtr,
	...
	if ( keyCode == 0 ) {
	    ...
	    // ####### begin Gilbert 25/7 ##########//
	    // while waitingfor input, flash the cursor
	    while( mouse.is_key_event() ? ((keyCode = mouse.key_code),mouse.get_event(),0) : 1 ) {
		// ####### end Gilbert 25/7 ##########//
...
int Get::store_and_valid( int keyCode, char* dataBuf, char* pictBuf, GetFld* getPtr,
	...
	// ####### begin Gilbert 24/7 ########//
	// && keyCode != KEY_CTRL_Q )
	if ( getPtr->valid_ptr  && keyCode != KEY_ESC ) {
	    // ####### end Gilbert 24/7 ########//
	    if (!( (*getPtr->valid_ptr)() ) )
		return 0;
	}
