//Filename    : OGRPDMTH.CPP
//Description : Object GraphDoubleBarMonth
//Owner			: Kevin(Ho)
...
#define MAX_LEN_LABEL "00,000,000"
...
static int info_graph_doublebar_month;

//-------- Begin of function GraphDoubleBarMonth::GraphDoubleBarMonth ----//
//!
GraphDoubleBarMonth::GraphDoubleBarMonth() {
    init_flag = 0;
    graph_bitmap = NULL;
}

//---------- End of function GraphDoubleBarMonth::GraphDoubleBarMonth ----//
...
//-------- Begin of function GraphDoubleBarMonth::init --------//
//!
...
//!
void GraphDoubleBarMonth::init(int x1, int y1, int x2, int y2,
...
//------------- End of function GraphDoubleBarMonth::init -----------//

//-------- Begin of function GraphDoubleBarMonth::deinit ------------//
//!
void GraphDoubleBarMonth::deinit() {
    if (graph_bitmap) {
...
//------------- End of function GraphDoubleBarMonth::deinit ---------//

//-------- Begin of function GraphDoubleBarMonth::set_font ------------//
//!
void GraphDoubleBarMonth::set_font(Font *fontPtr) {
    if (!init_flag)
	return;
...
//------------- End of function GraphDoubleBarMonth::set_font ---------//

//-------- Begin of function GraphDoubleBarMonth::calc_pos ------------//
//!
//! calculate screen positions
//! should be called after changing display settings
//!
void GraphDoubleBarMonth::calc_pos() {
    if (legend_array) {
...
//------------ End of function GraphDoubleBarMonth::calc_pos ---------//

//-------- Begin of function GraphDoubleBarMonth::paint --------------//
//!
void GraphDoubleBarMonth::paint() {
    if (transparent_flag) {
...
//------------- End of function GraphDoubleBarMonth::paint -----------//

//-------- Begin of function GraphDoubleBarMonth::refresh ------------//
//!
void GraphDoubleBarMonth::refresh() {
    if (transparent_flag) {
...
//------------- End of function GraphDoubleBarMonth::refresh ---------//

//---------- Begin of function GraphDoubleBarMonth::draw_legend ------------//
//!
void GraphDoubleBarMonth::draw_legend() {
    for (int i = 0; i < series_num; i++) {
...
//------------ End of function GraphDoubleBarMonth::draw_legend ------------//

//---------- Begin of function GraphDoubleBarMonth::draw_axis_label --------//
//!
void GraphDoubleBarMonth::draw_axis_label() {
...
//------------ End of function GraphDoubleBarMonth::draw_axis_label --------//

//---------- Begin of function GraphDoubleBarMonth::find_scale -------------//
//!
void GraphDoubleBarMonth::find_scale() {
    double data, maxVal = 0.0,minVal = 0.0;

    for (int i = 0; i < series_num * HISTORY_MONTH_COUNT; i++) {
...
//------------ End of function GraphDoubleBarMonth::find_scale -------------//

//---------- Begin of function GraphDoubleBarMonth::draw_scale -------------//
//!
void GraphDoubleBarMonth::draw_scale() {
    find_scale();

    //------ Draw y-axis -------//
    vga_back.bar(series_x1, series_y1-COMMON_OFFSET*2, series_x1+1, series_y2+COMMON_OFFSET, axis_color);
    ...
    //------ Draw x-axis -------//
    vga_back.bar(series_x1, xAxisPos+COMMON_OFFSET, series_x2+1, xAxisPos+2*COMMON_OFFSET, axis_color);

    // show x-axis scale

    int shown_month=*data_num;                      // max number of months will be shown

    info_graph_doublebar_month=                     // depending on info.graph_month, number of months will be shown
	(info.graph_month-2>shown_month)?
	shown_month:
    info.graph_month-2;
    ...
    x_axis_step = ((float)series_x2-series_x1) / (shown_month+1);

    double actual_start = actual_end-(double)(info_graph_doublebar_month)/12.0;
    // if finanical year
    // draw yr.i at i+(8.0/12.0) which is > actual_start
    // then i > actual_start-(8.0/12.0)
    // double financialYearOffset = ((financial.fiscal_year_start_month-1.0)/12.0); // for year start in sept
    double financialYearOffset = 8.0/12.0;          // for year start in sept
    int show_start = (int)(actual_start - financialYearOffset) + 1;
    double show_year_offset = (double)show_start + financialYearOffset - actual_start;

    show_end=show_start+1;
    ...
    //------ Draw x-axis -------//
    for (int i = 0; i < 12 ; i++) {
	int this_x=(int)(series_x1+(double)(i*x_axis_step));
	vga_back.bar(
	    this_x,
    ...
    for (int i = show_start; i <=show_end ; i+=displayInc) {
	int this_x;
	if((this_x = (int)(series_x1+((double)(i-show_start)+show_year_offset)*x_axis_step*12))<series_x2) {
		...
		xAxisPos+COMMON_OFFSET*2, axis_color);
	    font_ptr->center_put(
		this_x-maxLabelWidth/2-COMMON_OFFSET+4,
		xAxisPos+COMMON_OFFSET+2,
		this_x+maxLabelWidth/2-COMMON_OFFSET+4,
		xAxisPos+COMMON_OFFSET+2+font_ptr->max_font_height,
		m.format(i,16));
	}
    }

    //---- Draw this year indicators ----//
    if((this_month_indicator>=0)&&(this_month_indicator<*data_num))
	for(int j=series_y1;j<series_y2+COMMON_OFFSET;j+=8)
	    vga_back.bar(
		(int)(series_x1+(info_graph_doublebar_month-this_month_indicator+1)*x_axis_step),
		j,
		(int)(series_x1+(info_graph_doublebar_month-this_month_indicator+1)*x_axis_step),
		j+4,
		axis_color);
...
//------------ End of function GraphDoubleBarMonth::draw_scale -------------//

//---------- Begin of function GraphDoubleBarMonth::draw_series ------------//
//!
void GraphDoubleBarMonth::draw_series() {
    switch (data_type_flag) {
...
//------------ End of function GraphDoubleBarMonth::draw_series ------------//

//---------- Begin of function GraphDoubleBarMonth::draw_series_float ------------//
//!
void GraphDoubleBarMonth::draw_series_float() {
    float *dataArray = (float *) data_array;
    short currX, currY;
    int barWidth = (int)(x_axis_step/(series_num+1));

    for (int i = 0; i < series_num; i++) {
	dataArray-=(info_graph_doublebar_month);
	dataArray+=HISTORY_MONTH_COUNT;
	for (int j = 0; j < (info_graph_doublebar_month); j++) {
	    currX = (short)(series_x1 + (j+0.5) * x_axis_step + i * barWidth);
	    if (x_axis_pos)
		currY = xAxisPos+(short)(*dataArray * scaleStep/scaleInc)-COMMON_OFFSET+2;
	    else
		currY = xAxisPos-(short)(*dataArray * scaleStep/scaleInc)-COMMON_OFFSET+2; {

		//			if(*dataArray>0)
		vga_back.bar( (int)(currX-0.5*barWidth), currY,
			      (int)(currX+0.5*barWidth)-COMMON_OFFSET, xAxisPos,
			      series_color[i]);
	...
	if (value_flag)
	    draw_value(dataArray-1, vga_back.translate_color(series_color[i]), currX+COMMON_OFFSET-(int)(0.3f*x_axis_step), currY-font_ptr->max_font_height/2, DATA_FLOAT);
    }
}

//------------ End of function GraphDoubleBarMonth::draw_series_float ------------//

//---------- Begin of function GraphDoubleBarMonth::draw_series_double ------------//
//!
void GraphDoubleBarMonth::draw_series_double() {
	...
	for (int j = 0; j < (info_graph_doublebar_month); j++) {
	    currX = (short)(series_x1 + (j+0.5) * x_axis_step + i * barWidth);
	    if (x_axis_pos)
		currY = xAxisPos+(short)(*dataArray * (double)scaleStep/scaleInc)-COMMON_OFFSET+2;
	    else
		currY = xAxisPos-(short)(*dataArray * (double)scaleStep/scaleInc)-COMMON_OFFSET+2; {
	...
	if (value_flag)
	    draw_value(dataArray-1, vga_back.translate_color(series_color[i]), currX+COMMON_OFFSET-(int)(0.3f*x_axis_step), currY-font_ptr->max_font_height/2, DATA_DOUBLE);
    }
}

//------------ End of function GraphDoubleBarMonth::draw_series_double ------------//

//---------- Begin of function GraphDoubleBarMonth::draw_series_int ------------//
//!
void GraphDoubleBarMonth::draw_series_int() {
	...
	for (int j = 0; j < (info_graph_doublebar_month); j++) {
	    // shift half bar
	    currX = (short)(series_x1 + j * x_axis_step + i * barWidth + barWidth);
	    if (x_axis_pos)
		currY = (short)(xAxisPos+(int)(*dataArray * scaleStep/scaleInc))-COMMON_OFFSET+2;
	    else
		currY = (short)(xAxisPos-(int)(*dataArray * scaleStep/scaleInc))-COMMON_OFFSET+2; {
	...
	if (value_flag)
	    draw_value(dataArray-1, vga_back.translate_color(series_color[i]), currX+COMMON_OFFSET-(int)(0.3f*x_axis_step), currY-font_ptr->max_font_height/2, DATA_INT);
    }
}

//------------ End of function GraphDoubleBarMonth::draw_series_int ------------//

//---------- Begin of function GraphDoubleBarMonth::draw_series_long ------------//
//!
void GraphDoubleBarMonth::draw_series_long() {
	...
	if (value_flag)
	    draw_value(dataArray-1, vga_back.translate_color(series_color[i]), currX+COMMON_OFFSET-(int)(0.3f*x_axis_step), currY-font_ptr->max_font_height/2, DATA_LONG);
    }
}

//------------ End of function GraphDoubleBarMonth::draw_series_float ------------//

//---------- Begin of function GraphDoubleBarMonth::draw_value -------------------//
//!
void GraphDoubleBarMonth::draw_value(void *value, int color, short x, short y, int dataType) {
    char *valueString;
    switch (data_type_flag) {
    case DATA_FLOAT:
...
//---------- End of function GraphDoubleBarMonth::draw_value -------------------//
